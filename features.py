...
class FeatureExtractor():
    ...
    def _compute_formant_features(self, window):
        ...
        return np.histogram(self._compute_formants(window), range=(0,5500), bins = 15)[0] # returns dummy value; replace this with the features you extract
    ...
    def _compute_delta_coefficients(self, window, n=3):
        """
        ...
        results = []
        for i in range(n,mfccs.shape[0]-n):
            temp = []
            for j in range(1,n+1):
                temp.append(j*(mfccs[i+j]-mfccs[i-j]))
                """
                print(mfccs[i+j])
                print('-')
                print(mfccs[i-j])
                ...
                print(temp[-1])
                """
            results.append(np.sum(temp,axis=0))
